...
def updateStack(stackName, templateFile, s3Location):
  ...
  cfnClient.update_stack(
    ...
    Parameters=[
      ...
      {
        'ParameterKey': 'AuthLambdaArn',
        'ParameterValue': getStackOutput('auth-service', 'AuthLambdaArn')
      },
      {
...
def handler(event, context):
  ...
  try:
    ...
    if existsStack('location-service'):
      updateStack('location-service', '/tmp/service.json', s3Location)
    else:
      createStack('location-service', '/tmp/service.json', s3Location)
